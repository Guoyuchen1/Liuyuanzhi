...
from dataset_analysis import load_lbl2id_map, statistics_max_len_label
...
def info_nce_loss(self, features):
    # 对比损失计算，labels用来方便后面与矩阵做逐元素相乘的时候筛选正样本和负样本
...
def change_site(self, train_loader):
    scaler = GradScaler(enabled=self.args.fp16_precision)
    for epoch in range(self.epochs):
        for images, _ in batch(train_loader):
...
if __name__ == "__main__":
    ...
    # TODO set parameters
    base_data_dir = './mydata_2022/'  # 数据集根目录，请将数据下载到此位置
